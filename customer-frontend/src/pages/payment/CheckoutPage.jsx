...
export const CheckoutPage = () => {
  ...
  const handlePayment = async () => {
    ...
    if (selectedMethod === 'cash') {
      try {
        ...
        toast.success('Cash payment recorded. Please pay at counter upon pickup.');
        navigate('/dashboard/bookings');
      } catch (err) {
  ...
  const canPay = !isPaid && ['pending', 'confirmed'].includes(booking.status);
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-300/80">
                  This booking has been paid. No further action is needed.
                </p>
              </div>
            </div>
          )}
