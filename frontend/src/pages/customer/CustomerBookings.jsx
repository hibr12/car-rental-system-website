...
import { CalendarCheck, AlertCircle, XCircle, CheckCircle2, Eye, Filter } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">My Rental Bookings</h1>
          <p className="text-sm text-slate-400">View and manage your current and past vehicle reservations.</p>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
        {loading ? (
          ...
          <div className="text-center py-12 space-y-3">
            ...
            <p className="text-xs text-slate-500">You have no bookings matching the selected filter.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filteredBookings.map((booking) => (
                  <tr key={booking.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-4 px-4 text-xs text-slate-400">{formatDate(booking.pickup_date)}</td>
                    <td className="py-4 px-4 text-xs text-slate-400">{formatDate(booking.return_date)}</td>
          ...
          <div className="space-y-6 text-sm text-slate-300">
            <div className="grid grid-cols-2 gap-4 bg-slate-950 p-4 rounded-2xl border border-slate-800">
              <div>
                ...
                <p className="font-semibold text-white">{selectedBooking.pickup_location}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Return Location</p>
                <p className="font-semibold text-white">{selectedBooking.return_location}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Pickup Date</p>
                <p className="font-semibold text-white">{formatDate(selectedBooking.pickup_date, true)}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400">Return Date</p>
                <p className="font-semibold text-white">{formatDate(selectedBooking.return_date, true)}</p>
              </div>
            </div>
            ...
            <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-bold text-white">{selectedBooking.number_of_days} Days</span>
              </div>
              <div className="flex justify-between text-xs">
                <span>Price / Day:</span>
                <span className="font-bold text-white">{formatCurrency(selectedBooking.price_per_day)}</span>
              </div>
              <div className="flex justify-between text-xs pt-2 border-t border-slate-800">
                <span className="font-bold text-slate-100">Total Price:</span>
            ...
            {selectedBooking.notes && (
              <div>
                <p className="text-xs text-slate-400">Special Notes:</p>
                <p className="p-3 bg-slate-950 rounded-xl text-xs italic border border-slate-800">
                  {selectedBooking.notes}
                </p>
              </div>
            )}
        ...
        >
          <div className="space-y-4 text-center">
            ...
            <p className="text-xs text-slate-400">
              Are you sure you want to cancel booking <strong className="text-white">{selectedBooking.booking_reference}</strong>? This action cannot be undone.
            </p>

            <div className="pt-4 flex gap-3">
              <button
                onClick={() => setCancelModalOpen(false)}
                className="w-full py-2.5 rounded-xl border border-slate-800 text-slate-300 hover:bg-slate-800 text-xs font-semibold"
              >
                No, Keep Booking
              </button>
              <button
