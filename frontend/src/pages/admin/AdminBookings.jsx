  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-theme pb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-theme-primary tracking-tight">Booking Operations Workstation</h1>
          <p className="text-sm text-theme-muted">Confirm reservations, process vehicle pickups, and record returns.</p>
        </div>
      ...
      <div className="bg-theme-card border border-theme rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl">
        {loading ? (
          <div className="py-12 text-center text-theme-muted text-sm">Loading reservations...</div>
            ...
            <table className="w-full text-left text-sm text-theme-secondary">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filteredBookings.map((b) => (
                  <tr key={b.id} className="hover:bg-theme-hover transition-colors">
                    <td className="py-4 px-4 font-mono text-xs text-blue-400 font-bold">
                      {b.booking_reference}
                    </td>
                    <td className="py-4 px-4">
                      <p className="font-bold text-theme-primary">{b.user?.name || 'Customer'}</p>
                      <p className="text-[11px] text-theme-muted">{b.user?.email}</p>
                    </td>
                    <td className="py-4 px-4 font-medium text-theme-secondary">
                    ...
                    <td className="py-4 px-4 text-right space-x-2">
                      {b.status === 'pending' && (
                        <>
                          <button
                            ...
                            className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-theme-primary text-xs font-semibold"
                          >
                            Confirm
                          </button>
                          <button
                      ...
                      {b.status === 'confirmed' && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-theme-primary text-xs font-semibold flex items-center gap-1 ml-auto"
                        >
                      ...
                      {b.status === 'active' && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-500 text-theme-primary text-xs font-semibold flex items-center gap-1 ml-auto"
                        >
      ...
      {selectedBooking && (
        ...
        >
          <form onSubmit={handleRejectSubmit} className="space-y-4 text-xs">
            <div>
              <label className="block text-theme-secondary font-semibold mb-1">Reason for Rejection (Optional)</label>
              <textarea
                rows="3"
                value={rejectReason}
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3.5 rounded-2xl bg-rose-600 hover:bg-rose-500 text-theme-primary font-bold text-sm shadow-lg shadow-rose-600/25"
            >
